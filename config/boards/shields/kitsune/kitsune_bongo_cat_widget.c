...
void state_widget_wpm(struct kitsune_bongo_cat_widget *widget, int wpm) {
  LOG_DBG("anim state %d", current_anim_state);
  if (wpm < CONFIG_KITSUNE_BONGO_CAT_IDLE_LIMIT) {
  ...
  } else if (wpm < CONFIG_KITSUNE_BONGO_CAT_SLOW_LIMIT) {
...
lv_obj_t *
kitsune_bongo_cat_widget_obj(struct kitsune_bongo_cat_widget *widget) {
  return widget->obj;
}

int wpm_status_listener(const zmk_event_t *eh) {
  struct kitsune_bongo_cat_widget *widget;
...
ZMK_LISTENER(kitsune_bongo_cat_widget, wpm_status_listener)
ZMK_SUBSCRIPTION(kitsune_bongo_cat_widget, zmk_wpm_state_changed);
